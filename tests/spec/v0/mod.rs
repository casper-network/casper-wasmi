...
macro_rules! run_proposal_test {
    ($proposal: expr, $label: expr, $test_name: ident) => {
        #[test]
        fn $test_name() {
            self::run::proposal_spec($proposal, $label)
        }
    };
}
...
run_test!("load", wasm_load);
run_test!("local_get", wasm_local_get);
run_test!("local_set", wasm_local_set);
run_test!("local_tee", wasm_local_tee);
...
run_test!("memory_grow", wasm_memory_grow);
...
run_proposal_test!(
    "sign-extension-ops",
    "i64",
    wasm_proposal_sign_extension_i64
);
